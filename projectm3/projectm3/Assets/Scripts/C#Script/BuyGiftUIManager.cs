...
public class BuyGiftUIManager : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        // 강여진 대사 -1, -1
        myTable.Add("초콜릿1", ("입에 단 게 머리에 좋대!", 2, 6));
        myTable.Add("초콜릿2", ("쪼코렛...오오...", 0, 0));
        myTable.Add("초콜릿3", ("쪼!코!조!아!", 1, 1));
        myTable.Add("장미 꽃다발1", ("장미의 꽃말은 낭만적인 사랑이래!", 0, 0));
        myTable.Add("장미 꽃다발2", ("장미? 갑자기? 나한테? 고마워!", 2, 5));
        myTable.Add("장미 꽃다발3", ("뭐야 뭐야! 나를 닮은 꽃인 거야??", 1, 1));
        ...
        myTable.Add("로맨스 소설3", ("나도 가끔은 이런 소설의 주인공처럼 사는 상상을 해!", 1, 2));
        myTable.Add("헤드폰1", ("헤드셋은 목에 거는 게 정석이지!", 1, 1));
        myTable.Add("헤드폰2", ("오래 쓰면 머리가 눌려서 아파...", 2, 2));
        myTable.Add("헤드폰3", ("확실히 선이 있는 게 소리는 조금 더 좋은 것 같아!", 0, 0));
        myTable.Add("레스토랑 상품권1", ("오늘은 네가 쏘는 거야?!", 2, 5));
        myTable.Add("레스토랑 상품권2", ("가끔은 이런 것도 좋지!", 0, 0));
        myTable.Add("레스토랑 상품권3", ("마침 배고팠는데 잘됐다! 같이 가자!", 1, 1));
        myTable.Add("LP 레코드1", ("뭐야? 레코드? 이거 들으려면 턴테이블이 있어야 하지 않아?", 1, 6));
        myTable.Add("LP 레코드2", ("뭔가 굉장히 고급스러워 보이는데... 생각보다 엄청나게 커!", 0, 1));
        myTable.Add("LP 레코드3", ("이게 뭐야? LP? 이걸로 디제잉을 한다고? 슝슝?", 2, 5));
        myTable.Add("향수1", ("향수는 조금 낯설어!", 0, 6));
        myTable.Add("향수2", ("와! 이거 향 엄청 좋다!", 1, 5));
        myTable.Add("향수3", ("은은하게 퍼지는 게 나랑 잘 어울리는 거 같아!", 2, 1));
        // 진다영 대사 +2, +7
        myTable.Add("콜라1", ("콜라는 X카보다는 X시지.", 3, 8));
        myTable.Add("콜라2", ("뭐야? 난 제로 말고 그냥 콜라가 더 좋아!", 4, 9));
        myTable.Add("콜라3", ("이 썩는다고? 죽을래?", 5, 11));
        myTable.Add("비둘기 인형1", ("너 내가 비둘기 싫어하는 건 알아...? 아무튼 고마워.", 4, 10));
        myTable.Add("비둘기 인형2", ("이거 움직이는 건 아니지?", 3, 13));
        ...
        myTable.Add("컬처상품권1", ("이거로 책사는 사람은 잘 못 본 거 같아...", 4, 13));
        myTable.Add("컬처상품권2", ("오 머리 좀 썼네? 여기저기 다 쓸 수 있는 거잖아?", 3, 9));
        myTable.Add("컬처상품권3", ("이거 꼭 필요할 때는 없다니까...", 3, 11));
        myTable.Add("게임 CD1", ("CD? 오랜만에 보는데?", 3, 8));
        myTable.Add("게임 CD2", ("아직도 이런 걸 파는 곳이 있어?", 3, 13));
        ...
        myTable.Add("키 크는 영양제1", ("너 나 놀리는 거지?", 4, 10));
        myTable.Add("키 크는 영양제2", ("못 큰 게 아니고 안 큰 거야!", 5, 11));
        myTable.Add("키 크는 영양제3", ("죽는다. 진짜…?", 5, 10));
        myTable.Add("디지털 카메라1", ("카메라? 진짜로? 나 주는 거야?", 4, 8));
        myTable.Add("디지털 카메라2", ("핸드폰 말고 이런 카메라로 찍는 게 더 잘 나온다니까? ", 5, 9));
        myTable.Add("디지털 카메라3", ("너도...찍어줄까??", 4, 10));
        // 설나희 대사 +5, +13
        myTable.Add("불량식품1", ("이거 어릴 때 엄마가 못 먹게 했는데...", 8, 19));
        myTable.Add("불량식품2", ("가끔은 이런 것도 좋지...", 7, 15));
        myTable.Add("불량식품3", ("불량식품...", 6, 14));
        myTable.Add("츄르1", ("고양이는 츄르를 좋아해...", 9, 15));
        myTable.Add("츄르2", ("내가 먹을 거 아니야...", 6, 20));
        myTable.Add("츄르3", ("츄르...", 8, 14));
        myTable.Add("고양이 키링1", ("고양이가 세상을 구할 거야...", 7, 17));
        myTable.Add("고양이 키링2", ("샴인가봐 얼굴이 검은색이네...", 8, 16));
        myTable.Add("고양이 키링3", ("고양이...", 6, 14));
        myTable.Add("도시괴담 100선1", ("괴담은...별로 안 무서워하는데...", 9, 18));
        myTable.Add("도시괴담 100선2", ("지하철이 멈추지 않는 간이역의 비밀...", 8, 14));
        myTable.Add("도시괴담 100선3", ("도시 괴담...", 6, 14));
        myTable.Add("캔버스1", ("너도 한 장 그려줄까...?", 9, 16));
        myTable.Add("캔버스2", ("확실히 스케치북보다 캔버스에 그리는 게 더 좋아...", 7, 15));
        myTable.Add("캔버스3", ("캔버스...", 6, 14));
        myTable.Add("죽도1", ("우리 집에 진검도 있어...", 6, 15));
        myTable.Add("죽도2", ("죽도로 죽도록 맞아본 적 있어...?", 9, 17));
        myTable.Add("죽도3", ("죽도...", 7, 14));
        myTable.Add("비키니1", ("...무슨 뜻이야...?", 8, 20));
        myTable.Add("비키니2", ("이거...나한테는 조금 작아...", 6, 16));
        myTable.Add("비키니3", ("비키니...?", 6, 14));
        myTable.Add("오토바이 헬멧1", ("헬멧은 이미 있는데... 그래도 고마워...", 7, 15));
        myTable.Add("오토바이 헬멧2", ("내가 쓰는 거랑 디자인이 조금 다르네...", 8, 14));
